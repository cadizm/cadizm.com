...
class CreateUserTestCase(BaseTestCase):
    ...
    def test_valid_username_no_books(self):
        ...
        response = self.client.post(self.url, {'username': 'foo'})
    ...
    def test_username_already_exists(self):
        response = self.client.post(self.url, {'username': 'baz'})
        self.assertEquals(201, response.status_code)
        response = self.client.post(self.url, {'username': 'baz'})
        self.assertEquals(400, response.status_code)
    ...
    def test_valid_username_all_valid_book_ids(self):
        ...
        response = self.client.post(self.url, {'username': 'baz', 'book_ids': book_ids})
    ...
    def test_valid_username_all_invalid_book_ids(self):
        ...
        response = self.client.post(self.url, {'username': 'baz', 'book_ids': invalid_book_ids})
        result = response.json()['result']
    ...
    def test_valid_username_mixed_valid_invalid_book_ids(self):
        ...
        response = self.client.post(self.url, {'username': 'baz', 'book_ids': book_ids + invalid_book_ids})
        result = response.json()['result']
        self.assertEqual(set(book_ids), set(result['book_ids']))
        self.assertEqual(set(invalid_book_ids), set(result['invalid_book_ids']))
    ...
    def test_username_format(self):
        # nominal
        self.assertEqual(201, self.client.post(self.url, {'username': 'roys2017'}).status_code)

        # max username length
        username = 'foobarbazbammabzabraboof_123-456'
        self.assertEqual(32, len(username))
        self.assertEqual(201, self.client.post(self.url, {'username': username}).status_code)

        # invalid username length
        username += 'z'
        self.assertEqual(33, len(username))
        self.assertEqual(400, self.client.post(self.url, {'username': username}).status_code)

        # invalid username characters
        self.assertEqual(400, self.client.post(self.url, {'username': 'e{M(({TS'}).status_code)
