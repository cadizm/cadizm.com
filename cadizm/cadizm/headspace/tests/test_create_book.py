...
class CreateBookTestCase(BaseTestCase):
    ...
    def test_valid_title_author(self):
        ...
        response = self.client.post(self.url, dict(title=title, author=author))
    ...
    def test_title_empty(self):
        response = self.client.post(self.url, dict(title='', author='Shakespeare'))
        self.assertEquals(400, response.status_code)
        self.assertEquals('Missing or empty title/author', response.json()['meta']['reason'])

    def test_title_missing(self):
        response = self.client.post(self.url, dict(author='Shakespeare'))
        self.assertEquals(400, response.status_code)
        self.assertEquals('Missing or empty title/author', response.json()['meta']['reason'])

    def test_author_empty(self):
        response = self.client.post(self.url, dict(title="A Midsummer Night's Dream", author=''))
        self.assertEquals(400, response.status_code)
        self.assertEquals('Missing or empty title/author', response.json()['meta']['reason'])

    def test_author_missing(self):
        response = self.client.post(self.url, dict(title="A Midsummer Night's Dream"))
        self.assertEquals(400, response.status_code)
        self.assertEquals('Missing or empty title/author', response.json()['meta']['reason'])

    def test_invalid_lengths(self):
        response = self.client.post(self.url, dict(title='a', author=('b' * 129)))
        self.assertEquals(400, response.status_code)
        self.assertEquals('Invalid title/author length', response.json()['meta']['reason'])

        response = self.client.post(self.url, dict(title=('a' * 129), author='b'))
        self.assertEquals(400, response.status_code)
        self.assertEquals('Invalid title/author length', response.json()['meta']['reason'])
